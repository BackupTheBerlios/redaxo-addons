	...
	function showForm()
	{
		...
		if ($this->form_show || $this->ShowFormAlways)
		{
			...
			for ($i=0;$i<$this->counter;$i++)
			{
				...
				$addcolspawn = 0;
				if ($this->cols[$i]<$maxcount) $addcolspawn = 4;
				...
				case("mediaButton"):
						...
						$ausgabe .= "<input type=hidden name=REX_MEDIA_DELETE_1 value=0 id=REX_MEDIA_DELETE_1>";
						$ausgabe .= "<table><tr>
									<td><input type=text size=30 name=$name value=\"$value\" id=REX_MEDIA_1 readonly=readonly></td>
				...
				case("wysiwyg"):
					...
					$wysiwyg = new rex_wysiwyg_editor;
					$wysiwyg->id 		 = 1;
					...
					$wysiwyg->styles  	 = "";
					$wysiwyg->lang  	 = "";
					$wysiwyg->buttonrow1 = "bold,italic,underline,strikethrough,separator,justifyleft,justifycenter,justifyright,justifyfull,separator,bullist,numlist,link,linkHack,unlink,insertEmail,separator,removeformat,pasteRichtext,code"; // ,outdent,indentstyleselect,separator,
					$wysiwyg->buttonrow2 = " "; // ,separator,image
					$wysiwyg->buttonrow3 = " "; // tablecontrols, separator, visualaid
					$wysiwyg->buttonrow4 = " "; // rowseparator,formatselect,fontselect,fontsizeselect,forecolor,charmap
					...
					$ausgabe .= "<td class=grey colspan=".(1+$addcolspawn)."></td>";
					$ausgabe .= "";
					break;
			...
			$ausgabe .= "<td align=left class=dgrey colspan=".($addcolspawn+1)."><input type=submit value='".$this->submit_value."'></td>\n\n";
			$ausgabe .= "</tr>\n\n";
